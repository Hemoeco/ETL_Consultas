...
GO
CREATE   VIEW [dbo].[MovimientosOrdenados] AS
...
'FAC' + convert(varchar,T0.FACTURASNUMERO) AS cIdDocumento,
	CASE 
    ...
    rtrim(T0.DESCRIPCION) + CASE WHEN isnull(T5.ADUANA,isnull(T2.ADUANA, '')) <> '' THEN N', Aduana: ' + rtrim(isnull(T5.ADUANA,isnull(T2.ADUANA,''))) + N', Pedimento Importacion: ' + rtrim(isnull(T5.PEDIMENTOIMPORTACION,isnull(T2.PEDIMENTOIMPORTACION,''))) 
...
FROM [192.168.111.14].IT_Rentas.dbo.OperConFac AS T0
    ...
    ORDER BY T0.MTIPO, T0.CANTIDAD, T0.DIAS, T0.Importe, T0.DESCRIPCION
