...
CREATE OR ALTER VIEW [dbo].[Documentos_90Dias] AS
With DocFactura
...
WHERE T0.TOTAL <> 0
  ...
  AND T0.FOLIO2 = ''
  AND T0.PROCESADA = 'N'
  AND T0.FECHA >= dbo.fn_FechaIncluirAPartirDe()
)
-- Facturas
...
FROM Score.NotaDeCredito T0
	...
	INNER JOIN Comercial.Concepto T4 ON T4.CCODIGOCONCEPTO = 'NC' + case when left(T6.TIPO,8)='Devoluci' then 'V' else 'F' end  + REPLICATE('0', 2 - LEN(T0.IDCENTROOPERATIVO)) + CONVERT(varchar, T0.IDCENTROOPERATIVO) + CASE WHEN LEFT(T0.MONEDA, 1) = 'P' THEN 'N' ELSE 'E' END + '40'
...
WHERE T5.CIDDOCUMENTO IS NULL
	-- condiciones incluidas en 'Score.NotaDeCredito'
	AND T0.TOTAL <> 0
...
FROM Score.RM T0
	...
	INNER JOIN Comercial.Concepto T3 ON T3.CCODIGOCONCEPTO = 'FACP' + REPLICATE('0', 2 - LEN(T0.IDCENTROOPERATIVO)) + CONVERT(varchar, T0.IDCENTROOPERATIVO) + CASE WHEN LEFT(T0.MONEDA, 1) = 'P' THEN 'N' ELSE 'E' END
...
FROM Score.Devolucion T0
	...
	INNER JOIN Comercial.Concepto T3 ON T3.CCODIGOCONCEPTO = 'NCC' + REPLICATE('0', 2 - LEN(T0.IDCENTROOPERATIVO)) + CONVERT(varchar, T0.IDCENTROOPERATIVO) + CASE WHEN LEFT(T6.MONEDA, 1) = 'P' THEN 'N' ELSE 'E' END
...
FROM Score.OT T0
	INNER JOIN Score.ParaCentOper T1 ON T0.IDCENTROOPERATIVO = T1.IDCENTROOPERATIVO
	inner join Comercial.Concepto T2 on T2.CCODIGOCONCEPTO = 'ODT' + REPLICATE('0', 2 - LEN(T0.IDCENTROOPERATIVO)) + CONVERT(varchar, T0.IDCENTROOPERATIVO)
...
WHERE T0.FECHATERMINADO >= dbo.fn_FechaIncluirAPartirDe()
...
--	0 AS cNumParcia,
	...
	'' as cDestinatario,
	'' as cNumeroGuia
FROM Score.Requisicion T0
	INNER JOIN Score.ParaCentOper T1 ON T0.IDCENTROOPERATIVOORIGEN = T1.IDCENTROOPERATIVO
	inner join (select IDREQUISICION, FECHARECIBIDA from Score.ConReq group by IDREQUISICION, FECHARECIBIDA) T4 on T0.IDREQUISICION = T4.IDREQUISICION
	inner join Comercial.Concepto T2 on T2.CCODIGOCONCEPTO = 'REQ' + REPLICATE('0', 2 - LEN(T0.IDCENTROOPERATIVOORIGEN)) + CONVERT(varchar, T0.IDCENTROOPERATIVOORIGEN)
...
WHERE T4.FECHARECIBIDA >= dbo.fn_FechaIncluirAPartirDe()
  AND T3.cFolio IS NULL
...
--	0 AS cNumParcia,
	...
	'' as cDestinatario,
	'' as cNumeroGuia
FROM Score.EquipoRenta T0
	INNER JOIN Score.EquipoNuevo T1 ON T1.IDEQUIPORENTA = T0.IDEQUIPO
	INNER JOIN Score.Linea T2 ON T0.IDLINEA = T2.IDLINEA
	INNER JOIN Score.ParaCentOper T5 ON T0.IDCENTROOPERATIVO = T5.IDCENTROOPERATIVO
	INNER JOIN Comercial.Concepto T3 on T3.CIDDOCUMENTODE=2 and T3.CCODIGOCONCEPTO = 'PE' + REPLICATE('0', 2 - LEN(T0.IDCENTROOPERATIVO)) + CONVERT(varchar, T0.IDCENTROOPERATIVO)
	LEFT JOIN Comercial.Documento T4 ON T4.CIDDOCUMENTODE=2 AND T4.CSERIEDOCUMENTO='REN' AND T4.CFOLIO = T0.IDEQUIPO
WHERE T0.FECHAALTAHEMOECO >= dbo.fn_FechaIncluirAPartirDe()
  -- and dbo.fecha(T0.FECHAALTAHEMOECO) >= '20221201' Included above
...
Select * from Documentos_90Dias order by cIdDocumento