...
SELECT Concat('NC', T0.IDNOTASCREDITO) AS cIdDocumento,
	...
	rtrim(ComercialCodigoConcepto) AS cCodigoConcepto,
	CONVERT(varchar, T0.CLIENTESNUMERO) AS cCodigoCteProv,
	'N' + case when TipoConceptoAgrupado='Descuento' then 'F' else '' end + rtrim(T1.INICIALES) AS cSerieDocumento,
	0 AS cFolio,
	CASE WHEN LEFT(T0.Moneda, 1) = 'P' THEN 1 ELSE 2 END AS cIdMoneda,
	CASE WHEN (LEFT(T0.Moneda, 1) = 'P') THEN 1 ELSE isnull(T9.TipoCambio, T0.TIPOCAMBIO) END AS cTipoCambio,
	...
	1 AS cCantParci,
	'G02' as cCodConCba,
	case when TipoConceptoAgrupado in ('Descuento', 'Anticipo') then T0.SUBTOTAL else 0 end as cNeto,
	case when TipoConceptoAgrupado in ('Descuento', 'Anticipo') then case when T0.DESGLOSARIVA='N' then 0 else T0.PORCENTAJEIVA end else 0 end as cPorcentajeImpuesto1,
	case when TipoConceptoAgrupado in ('Descuento', 'Anticipo') then T0.IVA else 0 end as cImpuesto1,
	case when TipoConceptoAgrupado in ('Descuento', 'Anticipo') then T0.TOTAL else 0 end as cImporte,
	null as cUUID,
	case rtrim(Tipo2ConceptoAgrupado)
...
SELECT 'REQ' + CONVERT(varchar, T0.IDREQUISICION) AS cIdDocumento,
	...
	rtrim(ComerciaCodigoConcepto) AS cCodigoConcepto,
	'20902' AS cCodigoCteProv,
	InicialesCentOper AS cSerieDocumento,
	...
	'Requisición ' + CONVERT(varchar, T0.IDREQUISICION) + ', sucursal: ' + InicialesCentOper AS cObservaciones,
	'(Ninguno)' AS cCodigoAgente,
--	'' AS cNumeroG01, 
	'' AS cNumCtaPag,
	'' AS cMetodoPag,
...
SELECT 
	...
	CASE WHEN (LEFT(OD.Moneda, 1) = 'P') THEN 1 ELSE 1/OD.TIPOCAMBIO END * isnull(IVAPago,0) AS cImporteExtra1,
    '' AS cObserva01,
    '(Ninguno)' AS cCodigoAgente,
    '' AS cNumCtaPag,
	IdFormaPago as cMetodoPag,
    1 AS cCantParci,
	...
	null as cUUID,
	rtrim(CUENTABANCARIA) as cCodigoProyecto,
	CRUTACONTPAQ as cDestinatario,
	'' as cNumeroGuia
FROM Score.DepositoPorTimbrar as OD with (nolock)
	INNER JOIN Score.ClienteSucursal as CCS with (nolock) on OD.IdSucursalPago=CCS.IDSUCURSAL and OD.ClientesNumeroPago = CCS.IDNUMERO
