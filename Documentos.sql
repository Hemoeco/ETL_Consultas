...
/****** Object:  View [dbo].[Documentos]    Script Date: 07/03/2025 05:45:50 p. m. ******/
SET ANSI_NULLS ON
GO
...
--	CASE WHEN T4.METODODEPAGO = 'PPD' THEN 2 ELSE 1 END AS cCantParci,
	...
	'' as cDestinatario,
	'' as cNumeroGuia
FROM [192.168.111.14].IT_Rentas_Pruebas.dbo.OperFacturas T0
	INNER JOIN [192.168.111.14].IT_Rentas_Pruebas.dbo.ParaCentOper T1 ON T0.IDCENTROOPERATIVO = T1.IDCENTROOPERATIVO
	INNER JOIN [192.168.111.14].IT_Rentas_Pruebas.dbo.CataClientes T3 ON T3.IDCLIENTE = T0.CLIENTESNUMERO
	INNER JOIN [192.168.111.14].IT_Rentas_Pruebas.dbo.CataClientesSucursal T4 ON T4.IDNUMERO = T0.CLIENTESNUMERO AND T4.IDSUCURSAL = T0.IDSUCURSAL
	INNER JOIN adhemoeco_prueba.dbo.admConceptos AS T06 ON T06.CCODIGOCONCEPTO = CASE WHEN T0.CLIENTESNUMERO IN (10933, 670) THEN 'PE' ELSE 'FACC' END + REPLICATE('0', 2 - LEN(T0.IDCENTROOPERATIVO)) + CONVERT(varchar, T0.IDCENTROOPERATIVO) + CASE WHEN LEFT(MONEDA, 1) = 'P' THEN 'N' ELSE 'E' END  + '40'
	LEFT JOIN [192.168.111.14].IT_Rentas_Pruebas.dbo.CataObras T2 ON T2.IDCLIENTE = T0.CLIENTESNUMERO AND T2.NUMERO = T0.OBRASNUMERO
	LEFT JOIN adhemoeco_prueba.dbo.admDocumentos AS T08 ON T08.CIDCONCEPTODOCUMENTO = T06.CIDCONCEPTODOCUMENTO AND T08.CTEXTOEXTRA1 = CONVERT(varchar, T0.IDFACTURA) COLLATE Modern_Spanish_CI_AS
...
--  AND ISNULL(T4.FORMADEPAGO,'')<>''
  AND T0.FECHA >= 81819 -- 01/01/2025
  --AND T0.IDFACTURA > 475392
...
UNION SELECT 'NC' + CONVERT(varchar, T0.IDNOTASCREDITO) AS cIdDocumento,
	...
	'' as cDestinatario,
	'' as cNumeroGuia
FROM [192.168.111.14].IT_Rentas_Pruebas.dbo.OperNotasCredito T0
	INNER JOIN [192.168.111.14].IT_Rentas_Pruebas.dbo.ParaCentOper T1 ON T0.IDCENTROOPERATIVO = T1.IDCENTROOPERATIVO
	INNER JOIN [192.168.111.14].IT_Rentas_Pruebas.dbo.Operfacturas T2 ON T0.IDFACTURA = T2.IDFACTURA
	INNER JOIN (select S0.IDNOTASCREDITO, max(S0.TIPO) as TIPO, count(distinct S0.TIPO) as Num, max(S1.MTIPO) as TIPO2
				from [192.168.111.14].IT_Rentas_Pruebas.dbo.OperConNot S0
					inner join [192.168.111.14].IT_Rentas_Pruebas.dbo.OperConFac S1 on S0.IDCONFAC = S1.IDCONFAC
				group by S0.IDNOTASCREDITO
				having count(distinct S0.TIPO) = 1) as T6 on T6.IDNOTASCREDITO = T0.IDNOTASCREDITO
	INNER JOIN adhemoeco_prueba.dbo.admConceptos T4 ON T4.CCODIGOCONCEPTO = 'NC' + case when left(T6.TIPO,8)='Devoluci' then 'V' else 'F' end  + REPLICATE('0', 2 - LEN(T0.IDCENTROOPERATIVO)) + CONVERT(varchar, T0.IDCENTROOPERATIVO) + CASE WHEN LEFT(T0.MONEDA, 1) = 'P' THEN 'N' ELSE 'E' END + '40'
	LEFT JOIN [192.168.111.14].IT_Rentas_Pruebas.dbo.CataObras T3 ON T3.IDCLIENTE = T2.CLIENTESNUMERO AND T3.NUMERO = T2.OBRASNUMERO
	LEFT JOIN adhemoeco_prueba.dbo.admDocumentos T5 ON T5.CTEXTOEXTRA1 = CONVERT(varchar, T0.IDNOTASCREDITO) AND T5.CIDCONCEPTODOCUMENTO = T4.CIDCONCEPTODOCUMENTO AND T5.cCancelado = 0
	LEFT JOIN ctHemoeco_Renta_SA_de_CV_2016.dbo.TiposCambio AS T9 ON T9.Moneda = 2 AND T9.Tipo = 1 AND T9.Fecha = dbo.Fecha(T0.FECHA)
WHERE T0.TOTAL <> 0
...
FROM [192.168.111.14].IT_Rentas_Pruebas.dbo.OperRecepcionMercancia T0
	INNER JOIN [192.168.111.14].IT_Rentas_Pruebas.dbo.CataProveedores T2 ON T0.IDPROVEEDOR = T2.IDPROVEEDOR
	INNER JOIN [192.168.111.14].IT_Rentas_Pruebas.dbo.ParaPreferencias T4 ON T4.IDPREFERENCIAS = 1
	INNER JOIN [192.168.111.14].IT_Rentas_Pruebas.dbo.ParaCentOper T5 ON T0.IDCENTROOPERATIVO = T5.IDCENTROOPERATIVO
	INNER JOIN adhemoeco_prueba.dbo.admConceptos T3 ON T3.CCODIGOCONCEPTO = 'FACP' + REPLICATE('0', 2 - LEN(T0.IDCENTROOPERATIVO)) + CONVERT(varchar, T0.IDCENTROOPERATIVO) + CASE WHEN LEFT(T0.MONEDA, 1) = 'P' THEN 'N' ELSE 'E' END
	LEFT JOIN adhemoeco_prueba.dbo.admDocumentos T1 ON T1.CIDCONCEPTODOCUMENTO = T3.CIDCONCEPTODOCUMENTO AND T1.cFolio = T0.IDRECEPCIONMERCANCIA
...
WHERE year(dbo.fecha(FECHARECEPCION)) >= 2022
  AND T0.Cerrada = 1
  AND T1.cFolio IS NULL
--  and dbo.fecha(T0.FECHARECEPCION) >='20191001'
...
SELECT 'DEV' + CONVERT(varchar, T0.IDDEVOLUCION) AS cIdDocumento,
	...
	'' as cDestinatario,
	'' as cNumeroGuia
FROM [192.168.111.14].IT_Rentas_Pruebas.dbo.OperDevoluciones T0
	INNER JOIN [192.168.111.14].IT_Rentas_Pruebas.dbo.CataProveedores T2 ON T0.IDPROVEEDOR = T2.IDPROVEEDOR
	INNER JOIN [192.168.111.14].IT_Rentas_Pruebas.dbo.ParaPreferencias T4 ON T4.IDPREFERENCIAS = 1
	INNER JOIN [192.168.111.14].IT_Rentas_Pruebas.dbo.ParaCentOper T5 ON T0.IDCENTROOPERATIVO = T5.IDCENTROOPERATIVO
	inner join [192.168.111.14].IT_Rentas_Pruebas.dbo.OperRecepcionMercancia T6 on T6.IDRECEPCIONMERCANCIA = T0.IDRECEPCIONMERCANCIA
	INNER JOIN adhemoeco_prueba.dbo.admConceptos T3 ON T3.CCODIGOCONCEPTO = 'NCC' + REPLICATE('0', 2 - LEN(T0.IDCENTROOPERATIVO)) + CONVERT(varchar, T0.IDCENTROOPERATIVO) + CASE WHEN LEFT(T6.MONEDA, 1) = 'P' THEN 'N' ELSE 'E' END
	LEFT JOIN adhemoeco_prueba.dbo.admDocumentos T1 ON T1.CIDCONCEPTODOCUMENTO = T3.CIDCONCEPTODOCUMENTO AND T1.CSERIEDOCUMENTO=rtrim(T5.INICIALES) and T1.cFolio = T0.IDDEVOLUCION
...
--	0 AS cNumParcia,
	...
	'' as cDestinatario,
	'' as cNumeroGuia
FROM [192.168.111.14].IT_Rentas_Pruebas.dbo.OperOrdenesTrabajo T0
	INNER JOIN [192.168.111.14].IT_Rentas_Pruebas.dbo.ParaCentOper T1 ON T0.IDCENTROOPERATIVO = T1.IDCENTROOPERATIVO
	inner join adhemoeco_prueba.dbo.admConceptos T2 on T2.CCODIGOCONCEPTO = 'ODT' + REPLICATE('0', 2 - LEN(T0.IDCENTROOPERATIVO)) + CONVERT(varchar, T0.IDCENTROOPERATIVO)
	LEFT JOIN adhemoeco_prueba.dbo.admDocumentos T3 ON T3.CIDCONCEPTODOCUMENTO = T2.CIDCONCEPTODOCUMENTO AND T3.cFolio = T0.NUMERO -- AND T3.CSERIEDOCUMENTO=rtrim(T1.INICIALES) 
WHERE dbo.fecha(T0.FECHATERMINADO) >= '20220101'
  AND dbo.fecha(T0.FECHATERMINADO) <= getdate() 
  AND T0.FACTURASNUMERO = 0
  and (select sum(CANTIDAD - CANTIDADDEVUELTA) from [192.168.111.14].IT_Rentas_Pruebas.dbo.OperOTRefacciones where ORDENESTRABAJONUMERO = T0.NUMERO) <> 0
...
FROM [192.168.111.14].IT_Rentas_Pruebas.dbo.OperRequisiciones T0
	INNER JOIN [192.168.111.14].IT_Rentas_Pruebas.dbo.ParaCentOper T1 ON T0.IDCENTROOPERATIVOORIGEN = T1.IDCENTROOPERATIVO
	inner join (select IDREQUISICION, FECHARECIBIDA from [192.168.111.14].IT_Rentas_Pruebas.dbo.OperConReq group by IDREQUISICION, FECHARECIBIDA) T4 on T0.IDREQUISICION = T4.IDREQUISICION
	inner join adhemoeco_prueba.dbo.admConceptos T2 on T2.CCODIGOCONCEPTO = 'REQ' + REPLICATE('0', 2 - LEN(T0.IDCENTROOPERATIVOORIGEN)) + CONVERT(varchar, T0.IDCENTROOPERATIVOORIGEN)
	LEFT JOIN adhemoeco_prueba.dbo.admDocumentos T3 ON T3.CIDCONCEPTODOCUMENTO = T2.CIDCONCEPTODOCUMENTO AND T3.CSERIEDOCUMENTO=rtrim(T1.INICIALES) AND T3.cFolio = T0.IDREQUISICION
...
UNION SELECT 'TR' + CONVERT(varchar, T0.IDEQUIPO) AS cIdDocumento,
	CONVERT(VARCHAR(10), dbo.fecha(T0.FECHAALTAHEMOECO), 101) AS cFecha,
	CONVERT(VARCHAR(10), dbo.fecha(T0.FECHAALTAHEMOECO), 101) AS cFechaVencimiento,
	CONVERT(VARCHAR(10), dbo.fecha(T0.FECHAALTAHEMOECO), 101) AS cFechaEntregaRecepcion,
	rtrim(T3.CCODIGOCONCEPTO) AS cCodigoConcepto,
	'20902' AS cCodigoCteProv,
...
FROM [192.168.111.14].IT_Rentas_Pruebas.dbo.CataEquiposRenta T0
	INNER JOIN [192.168.111.14].IT_Rentas_Pruebas.dbo.CataEquiposNuevos T1 ON T1.IDEQUIPORENTA = T0.IDEQUIPO
	INNER JOIN [192.168.111.14].IT_Rentas_Pruebas.dbo.CataLineas T2 ON T0.IDLINEA = T2.IDLINEA
	INNER JOIN [192.168.111.14].IT_Rentas_Pruebas.dbo.ParaCentOper T5 ON T0.IDCENTROOPERATIVO = T5.IDCENTROOPERATIVO
	INNER JOIN adhemoeco_prueba.dbo.admConceptos T3 on T3.CIDDOCUMENTODE=2 and T3.CCODIGOCONCEPTO = 'PE' + REPLICATE('0', 2 - LEN(T0.IDCENTROOPERATIVO)) + CONVERT(varchar, T0.IDCENTROOPERATIVO)
	LEFT JOIN adhemoeco_prueba.dbo.admDocumentos T4 ON T4.CIDDOCUMENTODE=2 AND T4.CSERIEDOCUMENTO='REN' AND T4.CFOLIO = T0.IDEQUIPO
...
SELECT 'D' + CONVERT(varchar, OD.IDDEPOSITO) AS cIdDocumento,
	...
	(select top(1) convert(varchar,CLIENTESNUMERO) from [192.168.111.14].IT_Rentas_Pruebas.dbo.OperPagos where DEPOSITOSNUMERO = OD.IDDEPOSITO) AS cCodigoCteProv,
	...
	'' as cNumeroGuia
FROM [192.168.111.14].IT_Rentas_Pruebas.dbo.OperDepositos OD
	INNER JOIN [192.168.111.14].IT_Rentas_Pruebas.dbo.ParaCentOper PCO ON OD.IDCENTROOPERATIVO = PCO.IDCENTROOPERATIVO
	inner join [192.168.111.14].IT_Rentas_Pruebas.dbo.OperPagos OP on OP.DEPOSITOSNUMERO= OD.IDDEPOSITO
	INNER JOIN [192.168.111.14].IT_Rentas_Pruebas.dbo.CataClientesSucursal CCS on OP.IDSUCURSAL=CCS.IDSUCURSAL and OP.CLIENTESNUMERO = CCS.IDNUMERO
	inner join [192.168.111.14].IT_Rentas_Pruebas.dbo.CataCuentasBancos CCB on CCB.IDCUENTABANCOS = OP.IDCUENTABANCOS
	left join (select OP.DEPOSITOSNUMERO,
				...
				from [192.168.111.14].IT_Rentas_Pruebas.dbo.OperPagos OP
					inner join [192.168.111.14].IT_Rentas_Pruebas.dbo.OperFacPag OFP on OFP.PAGOSNUMERO = OP.NUMERO
					inner join [192.168.111.14].IT_Rentas_Pruebas.dbo.OperFacturas T2 on OFP.FACTURASNUMERO = T2.IDFACTURA
				where T2.TOTAL <> 0
				group by OP.DEPOSITOSNUMERO) TX on TX.DEPOSITOSNUMERO = OD.IDDEPOSITO
	INNER JOIN adhemoeco_prueba.dbo.admParametros M0 on M0.CIDEMPRESA>0
	left join adhemoeco_prueba.dbo.admDocumentos M8 on M8.CIDDOCUMENTODE=9 and M8.CSERIEDOCUMENTO='P' + rtrim(PCO.INICIALES) and M8.cfolio = OD.IDDEPOSITO
where dbo.fecha(OD.FECHA) >='20220101'
 and OD.TIMBRAR='S'
 and M8.CIDDOCUMENTO is null
 --AND OD.IDDEPOSITO=386326
 --and '10' <> case when TX.PUE>0 or OD.IMPORTE*CASE WHEN (LEFT(OD.Moneda, 1) = 'P') THEN 1 ELSE OD.TIPOCAMBIO END < 10 then '10' else 'PPD' end  -- se pone para poder contabilizar los pagos PUE
 */
GO
