...
GO
CREATE VIEW [dbo].[Movimientos_Prod] AS
SELECT 'FAC' + convert(varchar,T0.FACTURASNUMERO) AS cIdDocumento,
	CASE WHEN T0.IDEQUIPONUEVO + T0.IDEQUIPOUSADO <> 0 or rtrim(T0.DELAL)='Arrendadora' THEN 'MOD' + convert(varchar,T0.IDLINEA)
	...
	case when T1.DESGLOSARIVA='N' then 0 else T1.PORCENTAJEIVA end AS cPorcentajeImpuesto1,
