...
CREATE VIEW [dbo].[MovimientosSaul] AS
SELECT top 100 'FAC' + convert(varchar,T0.FACTURASNUMERO) AS cIdDocumento,
	CASE WHEN T0.IDEQUIPONUEVO + T0.IDEQUIPOUSADO <> 0 THEN 'MOD' + convert(varchar,T0.IDLINEA)
		ELSE CASE WHEN T0.IDREFACCION <> 0 THEN 'REF' + convert(varchar,T0.IDREFACCION)
			 ELSE CASE WHEN MTIPO = 'Renta Equipo' then 'REN' ELSE 'SRV' END
			 END
	END AS cCodigoProducto,
	case when T0.MTIPO = 'Renta Equipo' then T0.DIAS else T0.CANTIDAD end AS cUnidadesCapturadas,
	(1.00 + case when T1.DESGLOSARIVA='S' then 0 else T1.PORCENTAJEIVA end/100) * T0.PRECIOUNITARIO AS cPrecioCapturado,
	convert(decimal(15,2), case when T0.DIAS<>0 and T0.DELAL <> 'OTROS' then T0.DIAS else T0.CANTIDAD end * (case when T1.DESGLOSARIVA='N' then 0 else T1.PORCENTAJEIVA/100 end) * T0.PRECIOUNITARIO) AS cImpuesto1,
	case when T1.DESGLOSARIVA='N' then 0 else T1.PORCENTAJEIVA end AS cPorcent01,
	'0' + convert(varchar,T1.IDSUCURSAL) +
		case when T0.IDEQUIPONUEVO <> 0 then 'ENUE'
    ...
    rtrim(T0.DESCRIPCION) + CASE WHEN 1=0 and isnull(T2.ADUANA, '') <> '' THEN ', Aduana: ' + rtrim(isnull(T2.ADUANA,'')) + ', Ped Imp: ' + rtrim(isnull(T2.PEDIMENTOIMPORTACION,'')) 
    + ', Fec Ped: ' + isnull(CONVERT(VARCHAR(10), dbo.fecha(T2.FECHAPEDIMENTO), 103),'') ELSE '' END  AS cObservaMov,
	--CASE WHEN T0.DIAS <> 0 THEN rtrim(T0.DELAL) ELSE '' END AS cTextoEx01,
	CASE WHEN T0.DIAS <> 0 THEN rtrim(SUBSTRING(T0.DELAL, 1, CHARINDEX('-', T0.DELAL))+' '+ substring(T0.DELAL, CHARINDEX('-', T0.DELAL)+1, LEN(T0.DELAL))) ELSE '' END AS cTextoExtra1,
	CASE WHEN T4.IDEQUIPO is not null then 'Num. Int.: ' + rtrim(T4.NUMEROINTERNO) else '' end AS cTextoExtra2,
	'' AS cTextoExtra3,
	case T0.DELAL 
	...
	'' as cSCMovto
FROM [192.168.101.15].IT_Rentas.dbo.OperConFac AS T0
	INNER JOIN [192.168.101.15].IT_Rentas.dbo.OperFacturas AS T1 ON T0.FACTURASNUMERO = T1.IDFACTURA
	LEFT JOIN [192.168.101.15].IT_Rentas.dbo.CataEquiposNuevos T2 ON T2.IDEQUIPO = T0.IDEQUIPONUEVO
	LEFT JOIN [192.168.101.15].IT_Rentas.dbo.CataEquiposRenta T4 on T4.IDEQUIPO = T0.IDEQUIPORENTA
	LEFT JOIN [192.168.101.15].IT_Rentas.dbo.CataEquiposUsados AS T5 ON T0.IDEQUIPOUSADO = T5.IDEQUIPO
	LEFT JOIN [192.168.101.15].IT_Rentas.dbo.OperOTRefacciones AS T3 ON T0.OTRLLAVEAUTONUMERICA = T3.IDOTREFACCIONES
	--where T1.IDFACTURA = 403777
	order by T0.IDCONFAC
--UNION ALL SELECT 'NC' + convert(varchar,T0.IDNOTASCREDITO) AS cIdDocumento,
--	CASE WHEN T2.IDEQUIPONUEVO + T2.IDEQUIPOUSADO <> 0 THEN 'MOD' + convert(varchar, T2.IDLINEA)
...
--	case when T1.DESGLOSARIVA='N' then 0 else case when T1.PORCENTAJEIVA=11 then 16 else T1.PORCENTAJEIVA end end AS cPorcent01,
--	'0' + convert(varchar,T1.IDSUCURSAL) +
...
--    rtrim(T2.DESCRIPCION) AS cObservaMov, '' AS cTextoExtra1,
--	'' AS cTextoExtra2,
...
--	'' as cSCMovto
--FROM [192.168.101.15].IT_Rentas.dbo.OperConNot T0
--	INNER JOIN [192.168.101.15].IT_Rentas.dbo.OperNotasCredito T1 ON T0.IDNOTASCREDITO = T1.IDNOTASCREDITO
--	INNER JOIN [192.168.101.15].IT_Rentas.dbo.OperConFac T2 ON T0.IDCONFAC = T2.IDCONFAC
--	LEFT OUTER JOIN [192.168.101.15].IT_Rentas.dbo.CataEquiposNuevos AS S2 ON T2.IDEQUIPONUEVO = S2.IDEQUIPO AND T2.DELAL = 'Venta'
--	LEFT OUTER JOIN [192.168.101.15].IT_Rentas.dbo.CataEquiposRenta AS S3 ON T2.IDEQUIPORENTA = S3.IDEQUIPO AND T2.DELAL = 'Venta'
--	LEFT OUTER JOIN [192.168.101.15].IT_Rentas.dbo.CataEquiposUsados AS S4 ON T2.IDEQUIPOUSADO = S4.IDEQUIPO AND T2.DELAL = 'Venta'
--	LEFT OUTER JOIN [192.168.101.15].IT_Rentas.dbo.CataLineasSucursal AS S5 ON T2.IDLINEA = S5.IDLINEA AND T2.IDSUCURSAL = S5.IDSUCURSAL
--	LEFT OUTER JOIN [192.168.101.15].IT_Rentas.dbo.OperOTRefacciones AS S6 ON T2.OTRLLAVEAUTONUMERICA = S6.IDOTREFACCIONES
--WHERE T0.CANTIDAD > 0
...
--	T1.PORCENTAJEIVA AS cPorcent01,
...
--	0 AS cImporteExtra2,
--	'' as cSCMovto
--from [192.168.101.15].IT_Rentas.dbo.OperConRM T0
--	inner join [192.168.101.15].IT_Rentas.dbo.OperRecepcionMercancia T1 on T0.IDRECEPCIONMERCANCIA = T1.IDRECEPCIONMERCANCIA
--	left join [192.168.101.15].IT_Rentas.dbo.CataRefacciones T2 on T0.IDREFACCION = T2.IDREFACCION
--	left join [192.168.101.15].IT_Rentas.dbo.CataModelos T3 on T0.IDMODELO = T3.IDMODELO
--where year(dbo.fecha(T1.FECHARECEPCION)) >= 2017
--UNION ALL
...
--	0 AS cImporteExtra2,
--	'' as cSCMovto
--from [192.168.101.15].IT_Rentas.dbo.OperConDev T0
--	inner join [192.168.101.15].IT_Rentas.dbo.OperDevoluciones T1 on T0.IDDEVOLUCION = T1.IDDEVOLUCION
--	inner join [192.168.101.15].IT_Rentas.dbo.OperConRM S0 ON T0.IDCONRM = S0.IDCONRM
--	inner join [192.168.101.15].IT_Rentas.dbo.OperRecepcionMercancia S1 on S0.IDRECEPCIONMERCANCIA = S1.IDRECEPCIONMERCANCIA
--	left join [192.168.101.15].IT_Rentas.dbo.CataRefacciones T2 on T0.IDREFACCION = T2.IDREFACCION
--	left join [192.168.101.15].IT_Rentas.dbo.CataModelos T3 on T0.IDMODELO = T3.IDMODELO
--UNION ALL
...
--	0 AS cImporteExtra2,
--	'' as cSCMovto
--from [192.168.101.15].IT_Rentas.dbo.OperOTRefacciones T0
--	INNER JOIN [192.168.101.15].IT_Rentas.dbo.ParaCentOper T1 ON T0.IDCENTROOPERATIVO = T1.IDCENTROOPERATIVO
--	inner join [192.168.101.15].IT_Rentas.dbo.CataRefacciones T2 on T0.IDREFACCION = T2.IDREFACCION
...
--FROM [192.168.101.15].IT_Rentas.dbo.OperRequisiciones T0
--	inner join [192.168.101.15].IT_Rentas.dbo.OperConReq T1 on T0.IDREQUISICION=T1.IDREQUISICION
...
--	LEFT JOIN [192.168.101.15].IT_Rentas.dbo.CataEquiposNuevos AS S0 ON T1.IDEQUIPONUEVO = S0.IDEQUIPO
--	LEFT JOIN [192.168.101.15].IT_Rentas.dbo.CataEquiposRenta AS S1 ON T1.IDEQUIPORENTA = S1.IDEQUIPO
--	LEFT JOIN [192.168.101.15].IT_Rentas.dbo.CataEquiposUsados AS S2 ON T1.IDEQUIPOUSADO = S2.IDEQUIPO
--	LEFT JOIN [192.168.101.15].IT_Rentas.dbo.OperKardexAltas AS S3 ON T1.IDCONREQ = S3.IDCONREQ AND T1.IDREQUISICION = S3.DOCUMENTONUMERO
--	LEFT JOIN [192.168.101.15].IT_Rentas.dbo.CataLineas S4 ON S4.IDLINEA = ISNULL(S0.IDLINEA, 0) + ISNULL(S1.IDLINEA, 0) + ISNULL(S2.IDLINEA, 0)
--	LEFT JOIN [192.168.101.15].IT_Rentas.dbo.CataLineasSucursal S5 ON S4.IDLINEA = S5.IDLINEA AND T0.IDSUCURSALORIGEN = S5.IDSUCURSAL
...
--	0 AS cImporteExtra2,
--	A2.CSCALMAC2 as cSCMovto
--FROM [192.168.101.15].IT_Rentas.dbo.CataEquiposRenta T0
