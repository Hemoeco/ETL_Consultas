...
CREATE OR ALTER VIEW [dbo].[MovimientosOrderPers] AS
...
MovConFacPers as (
	SELECT top 100 percent CONCAT('FAC', pers.FacturasNumero) AS cIdDocumento,
			ccf.CodigoProducto AS cCodigoProducto,
		...
		from Score.ConFacPersPorTimbrar as pers
			...
			left join Comercial.Producto AS prod ON prod.CCODIGOPRODUCTO = ccf.codigoProducto
		Order by pers.FacturasNumero, Ordinal, con.MTIPO, pers.Cantidad, pers.ValorUnitario
...
-- a partir de aquí si funciona bien...
with CantidadConFac as (
	-- Convertir cCodigoProducto. Conversion Score
	Select IDCONFAC,
			case 
