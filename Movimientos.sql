...
/****** Object:  View [dbo].[Movimientos]    Script Date: 07/03/2025 03:31:16 p. m. ******/
SET ANSI_NULLS ON
GO
        ...
        Select IDCONFAC,
            case 
                when con.MTIPO = 'Renta equipo' then
                    case XML_ETIQUETA2 
                        when 'RENMES.' then 1 -- Caso espec√≠fico de convertir cualquier cantidad de dias a 1 mes.
                        else con.DIAS 
                    end
			...
			CASE
				...
				WHEN f.XML_ETIQUETA2 = 'MANIOBRA' AND MTIPO = 'servicio a obra' THEN f.XML_ETIQUETA2 -- ConversiÛn especial para 'MANIOBRA'
...
descripcionCorregida as (
	-- Quitar la primera palabra de la descripciÛn si coincide con el nombre de producto de comercial.
...
--	rtrim(T0.DESCRIPCION) AS cObservaMov,
	...
	case T0.DELAL 
		...
		when 'Refacci√≥n' then T0.CANTIDAD * ISNULL(T3.COSTOUNITARIO, 0)
